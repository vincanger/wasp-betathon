...
const SubmissionForm = ({ startDate, endDate }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      alert('Project submitted successfully! You will be notified once it is approved.');
      event.target.reset();
    } catch (e) {
      console.error('Error while submitting project', e);
      alert(e.message || 'Error while submitting project');
    }
  };
  ...
  return (
    <>
      {now < endDate && now > startDate ? (
                ...
                <form onSubmit={handleSubmit} method='POST'>
                  <div className='overflow-hidden shadow-md sm:rounded-md'>
                    ...
                    <div className='bg-gray-50 px-4 py-3 text-right sm:px-6'>
                      <button
                        type='submit'
                        disabled={isUploading || new Date() > endDate}
        ...
        <span className='m-8 mb-80 text-sm font-medium text-gray-500'>Submissions are closed at this time :(</span>
      )}
    </>
  );
};
